  ...
  useEffect(() => {
    ...
    const fetchNotices = async () => {
      ...
      try {
        ...
        setNotices(payload.notices || payload.items || payload || []);
        setPagination({
          page: payload.pagination?.page ?? page,
          pages: payload.pagination?.pages ?? 1,
          total: payload.pagination?.total ?? (payload.notices?.length || 0),
        });
      } catch (err) {
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            padding: '7px 14px',
            borderRadius: 8,
            border: `1px solid ${pinnedOnly ? '#06B6D4' : '#22222E'}`,
            background: pinnedOnly ? 'rgba(6,182,212,0.1)' : 'transparent',
            color: pinnedOnly ? '#06B6D4' : '#6B7280',
            fontSize: 13,
            cursor: 'pointer',
            fontWeight: 500,
        ...
        <span style={{ marginLeft: 'auto', fontSize: 13, color: '#4B5563' }}>
          {pagination.total} notice{pagination.total !== 1 ? 's' : ''}
        </span>
      </div>
        ...
        {!loading && !error && notices.length > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(340px,1fr))', gap: 20 }}>
            {notices.map((n, i) => (
              <div
                key={n._id}
                style={{ opacity: 0, animation: 'nb-fadeup 0.4s ease forwards', animationDelay: `${i * 60}ms` }}
              >
            ...
            {[...Array(pagination.pages)].map((_, i) => (
              <button
                ...
                style={{
                  ...
                  height: 36,
                  borderRadius: 8,
                  border: page === i + 1 ? 'none' : '1px solid #22222E',
                  background: page === i + 1 ? '#06B6D4' : 'transparent',
                  color: page === i + 1 ? '#000' : '#6B7280',
                  fontSize: 13,
                  fontWeight: 600,
                  cursor: 'pointer',
                }}
